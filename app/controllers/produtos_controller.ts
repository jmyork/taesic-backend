...
export default class produtossController {
  ...
  async store({ request, response, params, auth }: HttpContext) {
    try {
      ...
      const data = await this.service.create({ ...payload, company_alias: params.company_alias, user_id: auth.user?.id })
      ...
      // Captura erro de negocio. Não é permitido alterar um produto para serviço se ele tiver movimentações de estoque ou vendas associadas.
      if (error.code === 'NOT_ALLOWED_CHANGE_IS_SERVICE_TAG') {
        return response.badRequest({
          data: null,
  ...
  async update({ params, request, response, auth }: HttpContext) {
    try {
      ...
      const data = await this.service.update(params.id, { ...payload, user_id: auth.user?.id }, params.company_alias)
  ...
  // async registrar_produto_and_detalhes({ request, response }: HttpContext) {
  //   try {
  //     const payload = await request.validateUsing(CreateProdutoWithDetailsValidator)
  //     const data = await this.service.registrarProdutoAndDetalhes(payload)
  ...
  //     console.error('Erro ao registrar produto e detalhes:', error)
  //     return response.internalServerError({
  //       data: null,
  //       message: 'Erro interno do servidor',
  //       status: 500,
