...
import produto_recomendacoesService from '#services/produto_recomendacoes_service'
import {
  createproduto_recomendacoesValidator,
  updateproduto_recomendacoesValidator,
  ProdutoRecomendacaoQueryValidator,
} from '#validators/produto_recomendacoes_validator'
import { ProdutoRecomendacaoQueryDTO } from '#dtos/produto_recomendacoes_dto'

export default class produto_recomendacoessController {
  private service = new produto_recomendacoesService()

  // ==================== INDEX ====================
  async index({ request, response, params }: HttpContext) {
    try {
      const querySantized = await ProdutoRecomendacaoQueryValidator.validate(request.qs())
      const { page, limit, ...sanitezed } = querySantized

      const filter: ProdutoRecomendacaoQueryDTO = {
        ...sanitezed,
  ...
  async store({ request, response, params }: HttpContext) {
    try {
      const payload = await request.validateUsing(createproduto_recomendacoesValidator)
  ...
  async update({ params, request, response }: HttpContext) {
    try {
      const payload = await request.validateUsing(updateproduto_recomendacoesValidator, {
        meta: {
          id: params.id,
        },
      })
