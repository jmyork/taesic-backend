...
import produto_contraindicacoesService from '#services/produto_contraindicacoes_service'
import {
  createproduto_contraindicacoesValidator,
  updateproduto_contraindicacoesValidator,
  ProdutoContraIndicacaoQueryValidator,
} from '#validators/produto_contraindicacoes_validator'
import { ProdutoContraIndicacaoQueryDTO } from '#dtos/produto_contraindicacoes_dto'

export default class produto_contraindicacoessController {
  private service = new produto_contraindicacoesService()

  // ==================== INDEX ====================
  async index({ request, response, params }: HttpContext) {
    try {
      const querySantized = await ProdutoContraIndicacaoQueryValidator.validate(request.qs())
      const { page, limit, ...sanitezed } = querySantized

      const filter: ProdutoContraIndicacaoQueryDTO = {
        ...sanitezed,
  ...
  async store({ request, response, params }: HttpContext) {
    try {
      const payload = await request.validateUsing(createproduto_contraindicacoesValidator)
  ...
  async update({ params, request, response }: HttpContext) {
    try {
      const payload = await request.validateUsing(updateproduto_contraindicacoesValidator, {
        meta: {
          id: params.id,
        },
      })
