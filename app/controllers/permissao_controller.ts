...
import permissaoService from '#services/permissao_service'
import { createpermissaoValidator, updatepermissaoValidator } from '#validators/permissao_validator'

export default class permissaosController {
  private service = new permissaoService()

  // ==================== INDEX ====================
  async index({ request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PermissaoPolicy').authorize('index')
      ...
      console.error('Erro ao listar permissao:', error)
  ...
  async store({ request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PermissaoPolicy').authorize('store')

      const payload = await request.validateUsing(createpermissaoValidator)
  ...
  async show({ params, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PermissaoPolicy').authorize('show')
  ...
  async update({ params, request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PermissaoPolicy').authorize('update')
      const payload = await request.validateUsing(updatepermissaoValidator)
  ...
  async destroy({ params, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PermissaoPolicy').authorize('delete')
