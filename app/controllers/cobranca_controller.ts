...
import cobrancaService from '#services/cobranca_service'
import { createcobrancaValidator, updatecobrancaValidator } from '#validators/cobranca_validator'
...
export default class cobrancasController {
  ...
  async index({ request, params, response }: HttpContext) {
    try {
      ...
      console.error('Erro ao listar cobranca:', error)
  ...
  async store({ request, response }: HttpContext) {
    try {
      const payload = await request.validateUsing(createcobrancaValidator)
  ...
  async update({ params, request, response }: HttpContext) {
    try {
      const payload = await request.validateUsing(updatecobrancaValidator, {
        meta: {
          id: params.id,
        },
      })
