...
import subscricaoService from '#services/subscricao_service'
import {
  createsubscricaoValidator,
  updatesubscricaoValidator,
} from '#validators/subscricao_validator'

export default class subscricaosController {
  private service = new subscricaoService()
  ...
  async index({ request, params, response }: HttpContext) {
    try {
      ...
      console.error('Erro ao listar subscricao:', error)
  ...
  async store({ request, response }: HttpContext) {
    try {
      const payload = await request.validateUsing(createsubscricaoValidator)
  ...
  async update({ params, request, response }: HttpContext) {
    try {
      const payload = await request.validateUsing(updatesubscricaoValidator, {
        meta: {
          id: params.id,
        },
      })
  ...
  async destroy({ params, response }: HttpContext) {
    try {
      ...
      console.error('Erro ao remover subscricao:', error)
      return response.internalServerError({
        data: null,
        message: 'Erro interno do servidor',
        status: 500,
