...
export default class categorias_produtossController {
  private service = new categorias_produtosService()

  // ==================== INDEX ====================
  async index({ request, response, params }: HttpContext) {
    try {
      const querySantized = await CategoriaProdutoQueryValidator.validate(request.qs())
      const { page, limit, ...sanitezed } = querySantized

      const filter: CategoriaProdutoQueryDTO = {
        ...sanitezed,
  ...
  async store({ request, response, params }: HttpContext) {
    try {
      const payload = await request.validateUsing(createcategorias_produtosValidator)
