...
import papelService from '#services/papel_service'
import { createpapelValidator, updatepapelValidator } from '#validators/papel_validator'

export default class papelsController {
  ...
  async index({ request, response, auth, bouncer }: HttpContext) {
    try {
      ...
      console.error('Erro ao listar papel:', error)
  ...
  async store({ request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PapelPolicy').authorize('store')

      const payload = await request.validateUsing(createpapelValidator)
  ...
  async show({ params, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PapelPolicy').authorize('show')
  ...
  async update({ params, request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PapelPolicy').authorize('update')

      const payload = await request.validateUsing(updatepapelValidator, {
        meta: {
          id: params.id,
        },
      })
  ...
  async destroy({ params, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('PapelPolicy').authorize('delete')
