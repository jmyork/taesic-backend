...
import taxaIvaService from '#services/taxa_iva_service'
import { createtaxaivaValidator, updatetaxaivaValidator } from '#validators/taxa_iva_validator'

export default class taxaIvasController {
  private service = new taxaIvaService()

  // ==================== INDEX ====================
  async index({ request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('TaxaIvaPolicy').authorize('index')
      ...
      console.error('Erro ao listar taxa de iva:', error)
  ...
  async store({ request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('TaxaIvaPolicy').authorize('store')
      const payload = await request.validateUsing(createtaxaivaValidator)
  ...
  async show({ params, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('TaxaIvaPolicy').authorize('show')
      ...
      console.error('Erro ao buscar taxa de iva:', error)
      return response.internalServerError({ data: null, message: 'Erro interno do servidor', status: 500 })
    }
  }
  ...
  async update({ params, request, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('TaxaIvaPolicy').authorize('update')
      const payload = await request.validateUsing(updatetaxaivaValidator, { meta: { id: params.id } })
  ...
  async destroy({ params, response, bouncer }: HttpContext) {
    try {
      await bouncer.with('TaxaIvaPolicy').authorize('delete')
      ...
      console.error('Erro ao remover taxa de iva:', error)
      return response.internalServerError({ data: null, message: 'Erro interno do servidor', status: 500 })
    }
  }
}
