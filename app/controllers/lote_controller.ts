...
import loteService from '#services/lote_service'
import {
  createloteValidator,
  updateloteValidator,
  LoteQueryValidator,
} from '#validators/lote_validator'
import { LoteQueryDTO } from '#dtos/lote_dto'
...
export default class lotesController {
  ...
  async index({ request, response, params }: HttpContext) {
    try {
      const querySantized = await LoteQueryValidator.validate(request.qs())
      const { page, limit, ...sanitezed } = querySantized

      const filter: LoteQueryDTO = {
        ...sanitezed,
  ...
  async store({ request, response, auth, params }: HttpContext) {
    try {
      const payload = await request.validateUsing(createloteValidator)
  ...
  async update({ params, request, response }: HttpContext) {
    try {
      const payload = await request.validateUsing(updateloteValidator, {
        meta: {
          id: params.id,
        },
      })
