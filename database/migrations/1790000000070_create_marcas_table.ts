    ...
    this.defer(async (db) => {
      if (!(await temTabela(db, this.tableName))) {
        await db.schema.createTable(this.tableName, (table) => {
          ...
          table.primary(['id'])
          table.index(['deleted_at'], 'marcas_deleted_at_index')
          table.unique(['nome', 'empresa_id'], { indexName: 'marcas_nome_empresa_id_unique' })
          table
            .foreign(['empresa_id'], 'marcas_empresa_id_foreign')
            .references(['id'])
            .inTable('empresa')
            .onDelete('CASCADE')
            .onUpdate('NO ACTION')
